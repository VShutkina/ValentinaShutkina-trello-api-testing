...
public class Board {
    ...
    public String toString() {
        return new ToStringBuilder(this).append("id", id).append("name", name).append("desc", desc).append("descData", descData).append("closed", closed).append("idOrganization", idOrganization).append("invited", invited).append("limits", limits).append("memberships", memberships).append("pinned", pinned).append("starred", starred).append("url", url).append("prefs", prefs).append("invitations", invitations).append("shortLink", shortLink).append("subscribed", subscribed).append("labelNames", labelNames).append("powerUps", powerUps).append("dateLastActivity", dateLastActivity).append("dateLastView", dateLastView).append("shortUrl", shortUrl).append("idTags", idTags).append("datePluginDisable", datePluginDisable).toString();
    }

    @Override
    public int hashCode() {
        return new HashCodeBuilder().append(descData).append(idTags).append(pinned).append(labelNames).append(shortUrl).append(invited).append(dateLastActivity).append(datePluginDisable).append(shortLink).append(memberships).append(url).append(prefs).append(subscribed).append(starred).append(invitations).append(name).append(idOrganization).append(dateLastView).append(closed).append(id).append(limits).append(desc).append(powerUps).toHashCode();
    }
    ...
    public boolean equals(Object other) {
        ...
        return new EqualsBuilder().append(descData, rhs.descData).append(idTags, rhs.idTags).append(pinned, rhs.pinned).append(labelNames, rhs.labelNames).append(shortUrl, rhs.shortUrl).append(invited, rhs.invited).append(dateLastActivity, rhs.dateLastActivity).append(datePluginDisable, rhs.datePluginDisable).append(shortLink, rhs.shortLink).append(memberships, rhs.memberships).append(url, rhs.url).append(prefs, rhs.prefs).append(subscribed, rhs.subscribed).append(starred, rhs.starred).append(invitations, rhs.invitations).append(name, rhs.name).append(idOrganization, rhs.idOrganization).append(dateLastView, rhs.dateLastView).append(closed, rhs.closed).append(id, rhs.id).append(limits, rhs.limits).append(desc, rhs.desc).append(powerUps, rhs.powerUps).isEquals();
    }
